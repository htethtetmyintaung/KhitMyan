        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-6 col-12">
                    ...
                    <span>></span>
                    <a href="{{url('/newslist/ja')}}"> <strong>情報</strong> </a> 
                    <span>></span>
                    <span>{{$news_item->title_ja}}</span>
                </div>
            ...
            <div>
                <h2 class="text-align-center">{{$news_item->title_ja}}</h2>
                <hr>
            </div>
            ...
            <div>
                <div class="row mb-5">
                    ...
                    <div class="col-lg-6 col-12 new-detail">
                        <p>{!! $news_item->description_ja !!}</p>
                        <p>
                        @foreach($news_item->category as $category)
                            <span class="category">{{$category->name_ja}}</span>
                        @endforeach
                        <span class="created-by">{{$news_item->creator_ja}}</span><br><br>
                                ...
                                <div class="card w-100" >
                                    ...
                                    <div class="card-body">
                                        <a href="{{url('/news/'.$related_item->id)}}">
                                            <h5 class="card-title">{{ $related_item->title_ja }}</h5>
                                            <p class="card-text">{!! $related_item->description_ja !!}</p>
                                            <p>
                                                @foreach($related_item->category as $cate_name)
                                                    <span class="category">{{$cate_name->name_ja}}</span>
                                                @endforeach
                                                <span class="created-by">{{$related_item->creator_ja}}</span><br>

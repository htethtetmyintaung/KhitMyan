...
<section class="abouttitle padding-top-bottom">
    <div class="container">
        ...
        <div>
            <h2 class="text-align-center">{{$news->title_my}}</h2>
            <hr>
        </div>
            ...
            <div class="news-listpage mb-5">
                <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                    ...
                    @foreach($categories as $category)
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link " id="pills-{{ $category->id }}-tab" data-bs-toggle="pill" data-bs-target="#pills-{{ $category->id }}" type="button" role="tab" aria-controls="pills-{{ $category->id }}" aria-selected="true">{{ $category->name_my }}</button>
                        </li>
                            ...
                            <div class="col-lg-4 news-carousel news-item">
                                <div class="card-wrapper">
                                    <div class="card w-100" >
                                        ...
                                        <div class="card-body">
                                            <a href="{{url('/news/'.$item->id.'/my')}}">
                                                <h5 class="card-title">{{$item->title_my}}</h5>
                                                <p class="card-text">{!! $item->description_my !!}</p>
                                                <p>
                                                    @foreach($item->category as $cate_item)
                                                    <span class="category">{{$cate_item->name_my}}</span>
                                                    @endforeach
                                                    <span class="created-by">{{$item->creator_my}}</span><br>
